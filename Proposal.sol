...
contract ProposalContract{
    ...
    modifier activeProposal(){
        require(proposal_history[counter].is_active, "No active proposal is there");
        _;

    }
    modifier hasNotVoted(){
        require(!hasVoted[msg.sender],"Address has alredy voted");
        _;
    ...
    function createProposal(
        ...
        require(
            !proposal_history[counter].is_active, "Proposal is still active"
        );
        counter++;
    ...
    if (
        proposal.approve + proposal.reject +proposal.pass >=proposal.total_vote_to_end 
    ){
